(function () {
  ...
  C.behaviors.push({
    ...
    apply(proto) {
      ...
      proto.enter_dislikePage = function () {
        this.overlay.classList.add('active');
        ...
        this.countdownInterval = setInterval(function() {
          ...
          if (countdown > 0) {
            countdownEl.textContent = countdown;
            // Optionally show another message at 3, but since we show immediately, maybe skip
            // if (countdown === 3 && !messageShown) {
            //   messageShown = true;
            //   self.showBubble(self.pick(C.MESSAGES.dislikePage));
            // }
          } else {
            clearInterval(self.countdownInterval);
            // Send message to background to close the tab
      ...
      proto.exit_dislikePage = function () {
        this.overlay.classList.remove('active');
        if (this.frameId) { cancelAnimationFrame(this.frameId); this.frameId = null; }
        // Clean up countdown
        if (this.countdownInterval) {
      ...
      proto.attackLoop = function () {
        if (this.currentAction !== 'attacking' && this.currentAction !== 'rageQuit' && this.currentAction !== 'dislikePage') return;
